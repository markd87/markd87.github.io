...
\$$
f(x)=\sum_{n=1}^{\infty} A_n\sin(\pi n x)  cos(\omega_n t) 
$$

To obtain $$A_n$$ we multiply both sides by $$\sin(\pi m x)$$ and integrate on $$[0,1]$$. Here we use the property of the eigenfunctions or the normal modes which are guaranteed to be orthogonal in the sense of the dot product defined on our domain to be zero between different modes: $$\int_0^1 \sin(\pi n x) \sin(\pi m x) dx = \delta_{mn}\frac{1}{2}$$.
Therefore we obtain $$A_n=\int_0^1 f(x)\sin(\pi n x)$$. This gives the amplitude (or power) of each mode and frequency in the overall vibration of the string. 
...
\$$
u(r,\theta,t)=\sum_{m=0}^{\infty}\sum_{n=1}^{\infty}(C_{mn} \cos (m\theta)+D_{mn}\sin(m\theta))J_m(\lambda_{mn} r)(A_{mn} \cos (\lambda_{mn}t)+B_{mn}\sin(\lambda_{mn}t))
$$
